...
func (bq *BusinessQuery) QueryCoinProgram() *CoinProgramQuery {
	query := (&CoinProgramClient{config: bq.config}).Query()
...
func (bq *BusinessQuery) Clone() *BusinessQuery {
	...
	return &BusinessQuery{
		...
		withCoinProgram: bq.withCoinProgram.Clone(),
...
func (bq *BusinessQuery) WithCoinProgram(opts ...func(*CoinProgramQuery)) *BusinessQuery {
	query := (&CoinProgramClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withCoinProgram = query
	return bq
}
...
func (bq *BusinessQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Business, error) {
	...
	if query := bq.withCoinProgram; query != nil {
		if err := bq.loadCoinProgram(ctx, query, nodes, nil,
			func(n *Business, e *CoinProgram) { n.Edges.CoinProgram = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BusinessQuery) loadCoinProgram(ctx context.Context, query *CoinProgramQuery, nodes []*Business, init func(*Business), assign func(*Business, *CoinProgram)) error {
	...
	query.withFKs = true
	query.Where(predicate.CoinProgram(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(business.CoinProgramColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.business_coin_program
		if fk == nil {
			return fmt.Errorf(`foreign-key "business_coin_program" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "business_coin_program" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
