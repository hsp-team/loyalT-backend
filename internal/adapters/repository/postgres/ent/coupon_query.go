...
func (cq *CouponQuery) QueryOwner() *UserQuery {
...
func (cq *CouponQuery) QueryReward() *RewardQuery {
	query := (&RewardClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CouponQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (cq *CouponQuery) Clone() *CouponQuery {
	...
	return &CouponQuery{
		...
		withReward: cq.withReward.Clone(),
...
func (cq *CouponQuery) WithOwner(opts ...func(*UserQuery)) *CouponQuery {
	query := (&UserClient{config: cq.config}).Query()
...
func (cq *CouponQuery) WithReward(opts ...func(*RewardQuery)) *CouponQuery {
	query := (&RewardClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CouponQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Coupon, error) {
	...
	if cq.withOwner != nil || cq.withReward != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withOwner; query != nil {
		if err := cq.loadOwner(ctx, query, nodes, nil,
			func(n *Coupon, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withReward; query != nil {
		if err := cq.loadReward(ctx, query, nodes, nil,
			func(n *Coupon, e *Reward) { n.Edges.Reward = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CouponQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Coupon, init func(*Coupon), assign func(*Coupon, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Coupon)
	for i := range nodes {
		if nodes[i].user_coupons == nil {
			continue
		}
		fk := *nodes[i].user_coupons
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_coupons" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CouponQuery) loadReward(ctx context.Context, query *RewardQuery, nodes []*Coupon, init func(*Coupon), assign func(*Coupon, *Reward)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Coupon)
	for i := range nodes {
		if nodes[i].reward_rewards == nil {
			continue
		}
		fk := *nodes[i].reward_rewards
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "reward_rewards" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CouponQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(coupon.Table, coupon.Columns, sqlgraph.NewFieldSpec(coupon.FieldID, field.TypeUUID))
	_spec.From = cq.sql
